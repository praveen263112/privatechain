...
class Blockchain{
  ...
  // Get block height
    ...
    getBlockByHash(hash){
        return new Promise(function(resolve,reject){
            levelDB.getBlockByHash(hash).then(result=>resolve(result))
            .catch(error=>reject(error));
        })
    }
    
    //get block by wallet address used to store the star data
    getBlockByWalletAddress(walletAddress){
         return new Promise(function(resolve,reject){
            levelDB.getBlockByWalletAddress(walletAddress).then(result=>resolve(result))
            .catch(error=>reject(error));
        })
    }
